    ...
    public class EffectManager : AutoSingleton<EffectManager>
    {
        ...
        public ParticleSystem GetEffectInPool(string effectName)
        {
            if (!PoolManager.instance.IsPoolAlive(effectName))
                PoolManager.instance.AddPool<ParticleSystem>(() => { return Instantiate(ABManager.instance.LoadResource<GameObject>(effectABPackageName, effectName)).GetComponent<ParticleSystem>(); }, (u) => { u.gameObject.SetActive(false);u.transform.SetParent(this.transform); }, (u) => { u.gameObject.SetActive(false); },effectName);
