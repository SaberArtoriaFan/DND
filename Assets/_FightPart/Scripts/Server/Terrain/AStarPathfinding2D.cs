...
namespace XianXia.Terrain
{
    ...
    public class AStarPathfinding2D:MonoBehaviour
    {
            ...
            while (openList.Count > 0)
            {
                ...
                for (int i = 0; i < listCount; i++)
                {
                    ...
                    for (int j = 0; j < findDir.GetLength(0); j++)
                    {
                        ...
                        if (range < GetDistance(x, y, target.Position.x, target.Position.y, map)) continue;
        ...
        public static List<Node> FindTargetsInRange(Node center,float range,Func<Node,Node,bool>test,AStarPathfinding2D map, bool isTerrainObstale, bool isUnitObstale, bool containsSelf = true)
        {
            List<Node> res = new List<Node>();
            if (center == null) return null;
            if (containsSelf)
        ...
        public static List<Vector2Int> FindingPath(Vector3 startPos,Vector3 endPos, AStarPathfinding2D map,IDynamicObstacle unit, out Queue<float> pathG, GridType gridType=GridType.Ground, float stopRange = 0)
        {
            ...
            if (!AStarPathfinding2D.IsWalkable(endNode.Position, map))
            {
                //更换终点
                //Debug.Log("更换终点");
                endNode = FindNearestNode(startNode, endNode, map,false, stopRange > 0 ? stopRange : float.MaxValue);
                if (endNode == null)
                { Debug.LogError("cant find path to move!"); return new List<Vector2Int>(); }
