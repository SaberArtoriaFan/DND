...
namespace XianXia.Unit
{
    ...
    public class UnitMoveSystem :NormalSystemBase<LegOrgan>
    {
        ...
        public override void Awake(WorldBase world)
        {
            ...
            UnitMoveToNodeEvent= eventSystem.RegisterEvent<UnitBase, UnityEngine.Vector3>("UnitMoveToNodeEvent");
            UnitFindNoPathEvent = eventSystem.RegisterEvent<UnitBase>("UnitFindNoPathEvent");
        }
        ...
        public override void Start()
        {
            ...
            eventSystem.GetEvent<BodyOrgan,UnitBase>("UnitDeadBefore").AddAction(RemoveFollowTarget);
        ...
        private void RemoveFollowTarget(BodyOrgan body, UnitBase damage)
        {
            ...
            if (leg != null && leg.Follower==body.OwnerUnit)
            {
                UpdatePath(leg, null);
            }
        }
        ...
        public override void Update()
        {
            ...
            for(int i=0;i<allComponents.Count;i++)
            {
                ...
                {
                    FSM_State fSM_State = unitPosSystem.GetUnitAction(v.OwnerUnit);
        ...
        public void SetUnitLeg(LegOrgan leg, UnitBase target)
        {
            if (leg == null ) return;
            //Debug.Log("111");
            unitPosSystem.UpdateDynamicObstacle(unitPosSystem.GetGridItemByUnit(leg.OwnerUnit), false);
            ...
            leg.Follower = target;
            leg.Pos = unitPosSystem.GetPosByUnit(leg.OwnerUnit);
            leg.FollowerPos = leg.HasFollower ? unitPosSystem.GetPosByUnit(leg.Follower) : Vector3.zero;
        ...
        void UnitsMoveSystem(LegOrgan leg)
        {
            ...
            if (leg.Path.Count == 0)
            {
                ...
                if (( leg.Follower != null && !IsFollowEachOther(leg)&&Vector3.Distance(leg.FollowerPos, unitPosSystem.GetPosByUnit(leg.Follower)) > errorPositionCorrection))
                {
                ...
                leg.IsStand = true;
                //停下来了当然要更新障碍物
                unitPosSystem.UpdateDynamicObstacleReal(unitPosSystem.GetGridItemByUnit(leg.OwnerUnit));
                return;
            }
            //if(path.Count)
            ...
            Node myNode = unitPosSystem.GetGridItemByUnit(leg.OwnerUnit);
            ...
            if (distance < 0.1f)
            {
                ...
                // 检查是否需要更新路径
                if ((!IsFollowEachOther(leg)&&leg.Follower != null && Vector3.Distance(leg.FollowerPos, unitPosSystem.GetPosByUnit(leg.Follower)) > errorPositionCorrection))
                {
                    Debug.Log(leg.OwnerUnit+"//"+leg.Follower+ "跟随者移动，路径非法需要更新！！");
                    UpdatePath(leg, leg.Follower);
                    return;
            ...
            unitPosSystem.UpdateDynamicObstacleReal(myNode, false);
        ...
        internal void GoBackPos(UnitBase unitBase)
        {
            ...
            leg.Path.Add(unitPosSystem.GetPosByUnit(unitBase));
        }
            ...
            while (isCalculatingPath)
            {
                ...
                while (needCalculatePathLegs.Count > 0)
                {
                    ...
                    lock (leg)
                    {
                        ...
                        if (leg.Follower != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (UnitMainSystem.ActionPriorityLevel(unitPosSystem.GetUnitAction(leg.Follower)) && IsFollowEachOther(leg))
                                {
                                    ...
                                    {
                                        Node disNode = otherLeg.Path.Count > 0 ? AStarPathfinding2D.GetWorldPositionNode(otherLeg.Path[otherLeg.Path.Count - 1], map) : unitPosSystem.GetGridItemByUnit(otherLeg.OwnerUnit);
                                        if (AStarPathfinding2D.GetDistance(disNode, unitPosSystem.GetGridItemByUnit(leg.OwnerUnit), map) <= attackOrgan.AttackRange * map.NodeSize)
                                        {
                                            _ = needCalculatePathLegs.Dequeue();
                                            leg.IsFinding = false;
                                            continue;
                        ...
                        _ = needCalculatePathLegs.Dequeue();
                        if (leg.Path.Count == 0)
                            UnitFindNoPathEvent.Trigger(leg.OwnerUnit);
                        leg.IsFinding = false;
        ...
        private bool IsPathValid(Vector2Int pos,UnitBase unit)
        {
            ...
            if (unitPosSystem.GetUnitByGridItem(node) == null) return true;
            if (unitPosSystem.GetUnitByGridItem(node) != unit) return false;
            return true;
        }
